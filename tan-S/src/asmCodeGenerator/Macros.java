...
public class Macros {
     ...
     * @param offset amount to add to the base location before writing
     */
    public static void writeIOffset(ASMCodeFragment frag, int offset, int value) {
        frag.add(Duplicate); // [base base]
        if (offset != 0) {
            frag.add(PushI, offset); // [base base offset]
            frag.add(Add); // [base base+off]
        }
        frag.add(PushI, value); // [base base+off datum]
        frag.add(StoreI); // [base]
    }
