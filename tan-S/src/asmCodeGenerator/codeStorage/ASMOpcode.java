...
public enum ASMOpcode {
    ...
    Add, // adds top two elements of accumulator, leaving result on accumulator. [... a b] -> [...
         // a+b]
    ...
    PushPC, // [... a] -> [... a v] (where v is the (already incremented to next instruction) value
            // of the PC)
    ...
    Memtop, // pushes the size s of the memory onto the accumulator stack. This is an invalid
            // address,
    ...
    DataD, // takes a string (label) operand, and stores its value in the next 4 available memory
           // locations.

    PStack, // Nondestructively prints a copy of the current ASM accumulator stack. For debugging
            // purposes.
    ...
    // Note: all Labels and DLabels must be unique (they're all in the same namespace), and not
    // equal to an opcode,
    // but their definition does not need to precede their usage. The emulator does not warn you
    // when you have
    // identical Labels and/or DLabels; it silently deletes one of the definitions.
    // Valid Labels and DLabels consist of letters, digits, _, -, and $, and they don't start with a
    // digit.
    //
    ...
    public boolean takesString() {
        return this == PushD || this == DLabel || this == DataD || this == DataS
                || labelJumpOrCall();
    }
