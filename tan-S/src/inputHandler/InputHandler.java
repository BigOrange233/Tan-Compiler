...
/**
 * A line-based file reader that knows the filename and line number. Use one of the factories
 * <code>fromFilename(...)</code> to construct.
 * <p>
 * The lines returned by its iterator have their original line terminator replaced by the terminator
 * given to the factory (or the default of "\n" if the one-argument factory is used).
 * <p>
 * Although InputHandler is an Iterable on String, it permits only one Iterator. The current line
 * number of the InputHandler is the line number of the corresponding Iterator, if any.
 *
 */
public class InputHandler implements Iterable<String> {
     ...
     * Get the file name.
     * 
     * @return the filename that was passed to the constructor. This may be a relative or absolute
     *         file name.
     */
     ...
     * 
     * @return the one-based line number of the line that the <code>next()</code> last returned, or
